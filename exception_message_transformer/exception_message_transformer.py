...
from exception_message_transformer.helpers import (
    name_or_attribute_matches, name_or_attribute_matches_one_of,
    with_added_imports)
...
class ExceptionMessageTransformer(cst.CSTTransformer):
    ...
    def visit_ExceptHandler(self, node: cst.ExceptHandler) -> bool:
        if isinstance(node.name, cst.AsName):
            ...
            if not isinstance(node.name.name, cst.Name):
                raise TransformError(f"Expected ExceptHandler.name.name to be a cst.Name, but is a {type(node.name.name)}")
    ...
    def leave_ExceptHandler(self, node: cst.ExceptHandler, updated_node: cst.ExceptHandler) -> cst.ExceptHandler:
        self.except_name_stack.pop()
        return updated_node

    def leave_Attribute(self, node: cst.Attribute, updated_node: cst.Attribute) -> cst.Call:
